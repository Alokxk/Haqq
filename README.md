...
![React](https://img.shields.io/badge/React-18+-61DAFB?style=flat&logo=react&logoColor=61DAFB)

## How It Works

1. User describes their situation in the React frontend — in English or Hindi
...
- **Local embeddings** — `intfloat/multilingual-e5-large` runs via fastembed with no external API call per query. The ~1.1 GB model is downloaded once and cached; embeddings cost nothing at runtime
...
- Hindi is frontend-only — classification and analysis run in English
...
**Prerequisites:** Python 3.11+, Node.js 20+, Docker, [OpenRouter API key](https://openrouter.ai)
...
cd Haqq
cp .env.example .env
# Edit .env — fill in OPENROUTER_API_KEY and OPENROUTER_MODEL
...
python -m corpus.embed    # generate embeddings (~1.1 GB model downloads on first run)
```
...
*Haqq is not a substitute for legal advice. For court proceedings, consult a registered advocate.*
